...
def view_skills_for_a_role(jobroleId):
    ...
    return {"code": 400, "message": "No skills found"}
...
def check_skills_completed(jobroleId):
    ...
    return jsonify({"code": 200, "data": skills_of_role, "role": jobrole.JobRole_Name})
...
def create_a_skill():
    ...
    return jsonify({"code": 200, "data": skill.json()}), 201
...
def update_a_skill(skillId):
    ...
    return (
        jsonify(
            {"code": 404, "data": {"skillId": skillId}, "message": "Skill not found."}
        ),
        404,
    )
